...
import EventModal from './eventModal'; // Importing the EventModal component
...
const Calendar = () => {
    ...
    return (
        <div className="flex justify-center items-center h-screen h-screen bg-gradient-to-r from-blue-200 via-blue-500 to-blue-200">
            <div className="flex">
                <div className="w-80 bg-white bg-opacity-90 rounded-lg shadow-lg opacity-80">
                    {/* Calendar Header */}
                    <div className="flex justify-between items-center p-5 border-b border-gray-200">
                        <button
                            onClick={() => handleMonthChange(-1)}
                            className="w-[40px] h-[40px] text-center text-[16px] text-[#2d1ea0] bg-[#f8f7fa] rounded-[15px] cursor-pointer"
                    ...
                    <div className="flex flex-col">
                        ...
                        {/* Div to show weekdays & weekend names*/}
                        <div className="flex justify-between px-4 pt-3 text-[#394c96]">
                            {['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map((day, index) => (
                                <div
                                    className={`w-10 h-10 text-center font-semibold ${index === 0 || index === 6 ? 'text-[#FF0000]' : ''
                                        }`} // Change text color to red for Sunday (index 0) and Saturday (index 6)
                                    key={day}
                                >
                                    {day} {/* Day Labels */}
                                </div>
                        ...
                        {/* Div to show date numerals of month */}
                        <div className="flex flex-wrap px-4 pb-3">
                            {calendarDays.map((day, index) => {
                                ...
                                return (
                                    <div
                                        key={index}
                                        className={`w-10 h-10 text-center leading-10 ${day.isCurrentMonth ? (dayEvents ? getEventTypeClass(dayEvents.events[0]) : (day.isToday ? 'bg-blue-500 text-white rounded-lg font-bold cursor-pointer' : 'bg-white text-gray-700 hover:bg-gray-200 hover:text-indigo-600 transition duration-200 cursor-pointer')) : 'text-gray-400'}`}
                    ...
                    <div className="flex justify-evenly border-t border-gray-200 p-4">
                        <button
                            ...
                            className="bg-[#f8f7fa] rounded-[15px] px-[13px] py-[11px] text-[#394c96] cursor-pointer"
                ...
                {/* Color Legend Card */}
                {!selectedDay && (
                    <div className="w-40 bg-white bg-opacity-90 rounded-lg shadow-lg ml-4 opacity-80">
